                    ...
                    <table class="table">
                        ...
                        @foreach($carts as $item)
                            ...
                            <script type="text/javascript">
                                ...
                                cart.push({id: {{$item->id}}, quantity: 1, loc: ''});
                            </script>
                            <tr>
                                <td>
                                    <a type="button" class="btn btn-xs text-danger" title="remove item from list" onclick="var result = confirm('Are you very sure you want to remove this Item from the cart?');
                                        if(result){
                                    ...
                                    <form id="delete-cart{{$w}}" action="{{ route('delete.cart') }}" method="post" style="display:none;">
                                        {{csrf_field()}}
                                ...
                                <td>
                                    <div class="media">
                                        <div class="d-flex">
                                            <img src="{{ asset('/img/products/'. $item->product->images[0]->file_path) }}" width="50px" alt="">
                                        </div>
                                ...
                                <td>
                                    <h5>@if(session('currency') == 'Naira') {{$cur}}{{ number_format($item->cost_ngn, 2) }} @else {{$cur}}{{ number_format($item->cost_dol) }} @endif</h5>
                                </td>
                                <td>
                                    <div class="product_count">
                                        <input type="text" name="qty" id="sst{{ $w }}" maxlength="12" value="1" title="Quantity:"
                                        ...
                                        <button onclick="
                                            ...
                                            var amt = parseFloat('<?php
                                                        if(session('currency') == 'Naira') echo $item->cost_ngn;
                                                        else echo $item->cost_dol;
                                                        ?>');
                                ...
                                <td>
                                    <h5><?php
                                        ...
                                        if(session('currency') == 'Naira') echo number_format($item->cost_ngn, 2);
                                        else echo number_format($item->cost_dol, 2);
                                        echo '</span>';
                                        ?>
                                    </h5>
                                </td>
...
{{--                                <h5>Shipping</h5>--}}
                            ...
                            <td>
                                <div class="shipping_box">
                                    ...
                                    <select class="shipping_select" id="shipping" onchange="handleSelectChange(event)">
                        ...
                        <tr class="out_button_area">
                            ...
                            <td>
                                <div class="checkout_btn_inner d-flex align-items-center">
                                    ...
                                    <a class="primary-btn" href="#" onclick="
                                    event.preventDefault();
                                    document.getElementById('checkout').submit()">Proceed to checkout</a>
                                    <form class="form-inline" id="checkout" method="post" action="{{ route('checkout') }}" role="form">
                                        @csrf
        ...
        $.ajax({
            ...
            complete: function( response ) {
                if(response.status === 200){
                    ...
                    $('#coupon-box').html('<td></td><td></td><td></td><td><h5>Coupon</h5></td><td><h5>-'+ amt + '</h5></td>').show();
